...
```{r, echo=FALSE}
# Tabelle mit Spaltennamen und Datentypen erstellen
get_column_info <- function(df) {
  data.frame(
    Attribute = colnames(df),  # Spaltennamen als eigene Spalte
    Type = sapply(df, class),  # Datentypen jeder Spalte
    row.names = NULL
  )
}

# Ausgabe der Tabelle
print(get_column_info(gifts_age))
print(get_column_info(gifts_gender))
print(get_column_info(historical_spending))
print(get_column_info(data))
```
...
line_plot <- ggplot(longer, aes(x = Year, y = Percentage, color = Category, group = Category)) +
  ...
  labs(title = "Spending Trends by Category", 
       ...
       y = "Percentage", 
       color = "Category",
       caption = "Data source: TidyTuesday 2024-02-13") +
  theme_minimal()
...
ggplot(longer, aes(x = Year, y = Spending, shape = Category, color = Category)) +
    ...
    labs(title = "Trends of Spending by Category", x = "Year", y = "Average Spending in $", caption = "Data source: TidyTuesday 2024-02-13") +
    scale_x_continuous(
...
In conclusion, the COVID-19 pandemic had a notable impact on Valentine's Day spending behavior, leading to an increase in both the percentage of people celebrating the holiday and the average amount spent per person. Age plays a significant role in spending behavior, with older individuals less likely to spend money on Valentine's. Since the data covers only the year 2020, we cannot draw conclusions about trends over the entire COVID-19 pandemic or determine whether these trends persisted after the pandemic.

## Future Outlook

Future studies could explore the impact of the COVID-19 pandemic on other holidays and consumer behaviors. Additionally, analyzing the influence of other factors, such as income and relationship status, on Valentine's Day spending could provide further insights into consumer behavior trends.
