...
We observe that the number of positive tested people per day increases rapidly in the year 2020. Let's now have a look at the spending behaviour at this point in time.
...
Now, we want to investigate the connection between the different Age groups and their spending behavior on Valentines day. Therefore we Plot the percentage of each Age group with how much percent of people from this group spent money on valentines day. Then we fit a linear Model to this data to see if there is a linear connection between the Age and the amount of people from an age group spending money on Valentines day.
...
We can clearly see a downwards trend and the linear line fits the data pretty neat. That means that there is a clear, linear connection between age and spending behavior. This connection is a nearly perfect negative correlation. The older the people the less they tend to spend money on Valentines day.
...
To see how the above distribution changes over the years we can look at the following plot.
...
To make the trend lines more visible we exclude the actual data points to receive the below plot.
...
Interestingly, the popularity of Jewelry, Clothing, Candy and Gift cards increased significantly while the ones of Greeting Cards and Evenings are somewhat the same. 
